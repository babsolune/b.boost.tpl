<section id="module-news" class="category-{CATEGORY_ID} single-item">
	<header>
	...
	<article id="news-item-{ID}" class="news-item# IF C_NEW_CONTENT # new-content# ENDIF #" itemscope="itemscope" itemtype="https://schema.org/CreativeWork">
		...
		<div class="flex-between">
					...
					# ELSE #
						# IF NOT C_ID_CARD #
							<span class="pinned {AUTHOR_LEVEL_CLASS}"# IF C_AUTHOR_GROUP_COLOR # style="color:{AUTHOR_GROUP_COLOR}; border-color:{AUTHOR_GROUP_COLOR}" # ENDIF #>
								<i class="far fa-user" aria-hidden="true"></i> # IF C_AUTHOR_EXIST #<a itemprop="author" rel="author" class="{AUTHOR_LEVEL_CLASS}" href="{U_AUTHOR}" # IF C_AUTHOR_GROUP_COLOR # style="color:{AUTHOR_GROUP_COLOR}" # ENDIF #>{AUTHOR_DISPLAY_NAME}</a># ELSE #<span class="visitor">{AUTHOR_DISPLAY_NAME}</span># ENDIF #
							</span>
						# ENDIF #
					# ENDIF #
				# ENDIF #
			...
			# IF C_CONTROLS #
				<div class="controls align-right">
					# IF C_EDIT #
					<a href="{U_EDIT}" aria-label="{@common.edit}"><i class="far fa-fw fa-edit"></i></a>
					# ENDIF #
					# IF C_DELETE #
					<a href="{U_DELETE}" data-confirmation="delete-element" aria-label="{@common.delete}"><i class="far fa-fw fa-trash-alt"></i></a>
					# ENDIF #
				</div>
			# ENDIF #
		</div>
